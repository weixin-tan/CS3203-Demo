...
TEST_CASE("Basic check 1") {
	...
	correct_statement_one.statement_type = StatementType::kassign_stmt;
	correct_statement_one.stmt_no = 1;
	correct_statement_two.statement_type = StatementType::kprint_stmt;
	correct_statement_two.stmt_no = 2;
	...
	REQUIRE(statement_one.stmt_no == 1);
	REQUIRE(statement_one.statement_type == StatementType::kassign_stmt);

	Statement statement_two = stmtLst.getStmtAtIndex(1);
	REQUIRE(statement_two.stmt_no == 2);
	REQUIRE(statement_two.statement_type == StatementType::kprint_stmt);
}