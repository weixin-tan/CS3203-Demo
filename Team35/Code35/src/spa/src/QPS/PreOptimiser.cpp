...
void PreOptimiser::findConnectedRelationshipsAndAdd(int index1, std::unordered_map<int, std::deque<int>>* adjacencyList, const std::vector<RelationshipRef>& relationships){
...
int PreOptimiser::countNumberOfNonFixedEntity(const RelationshipRef& relationship){
    ...
    if (relationship.AssignmentEntity.eType != EntityType::NULL_ENTITY && !isFixedEntity(relationship.AssignmentEntity)){
        i = i + 1;
    }
    return i;
}
...
void PreOptimiser::traverseNeighbours(std::unordered_map<int, std::deque<int>> *adjacencyList, int currentNode, std::vector<int> *visited, std::queue<int> *myQueue){
...
RelationshipRefGroup PreOptimiser::traverseGraph(std::vector<int> *visited, const std::vector<RelationshipRef>& relationships,  std::unordered_map<int, std::deque<int>>* adjacencyList){
